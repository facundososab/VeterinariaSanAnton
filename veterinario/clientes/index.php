...
if (isset($_GET['searchCliente']) && !empty($_GET['searchCliente'])) {
  ...
  $total_clientes = $vet->totalClientesXBusqueda($filtro);
  if ($total_clientes == 0) {
    $_SESSION['mensaje'] = 'No se encontraron resultados';
    $_SESSION['msg-color'] = 'warning';
  } else {
    $clientes = $vet->getClientesXBusqueda($filtro, $empezar_desde, $tamano_paginas);
  }
} else {
  $total_clientes = $vet->totalClientes();
  if ($total_clientes == 0) {
    $_SESSION['mensaje'] = 'No hay clientes registrados';
    $_SESSION['msg-color'] = 'warning';
    return;
  }
  $clientes = $vet->getAllClientes($empezar_desde, $tamano_paginas);
}
