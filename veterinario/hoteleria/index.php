...
if (isset($_GET['searchHospedaje']) && !empty($_GET['searchHospedaje'])) {
  ...
  $total_hospedajes = $vet->totalHospedajesXBusqueda($filtro);
  if ($total_hospedajes == 0) {
    $_SESSION['mensaje'] = 'No se encontraron resultados';
    $_SESSION['msg-color'] = 'warning';
  } else {
    $hospedajes = $vet->getHospedajesXBusqueda($filtro, $empezar_desde, $tamano_paginas);
  }
} else {
  $total_hospedajes = $vet->totalHospedajes();
  $hospedajes = $vet->getAllHospedajes($empezar_desde, $tamano_paginas);
  if (empty($hospedajes)) {
