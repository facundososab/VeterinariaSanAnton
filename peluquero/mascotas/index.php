...
if ((isset($_GET['searchMascotas']) && !($_GET['searchMascotas'] == ''))) {
  $total_mascotas = $pelu->totalMascotasXBusqueda($_GET['searchMascotas']);
  if ($total_mascotas == 0) {
    $_SESSION['mensaje'] = 'No se encontraron resultados';
    $_SESSION['msg-color'] = 'warning';
  } else {
    $mascotas = $pelu->getMascotasXBusqueda($_GET['searchMascotas'], $empezar_desde, $tamano_paginas);
  }
} else {
  $total_mascotas = $pelu->totalMascotas();
  $mascotas = $pelu->getAllMascotas($empezar_desde, $tamano_paginas);
  if (empty($mascotas)) {
      ...
      <nav class="navbar">
        <div class="container-fluid justify-content-end">
          <form class="d-flex" role="search" id="formBuscadorMascotas" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="GET">
            <input class="form-control me-2" type="search" placeholder="Nombre o raza" aria-label="Buscar" name="searchMascotas" value="<?php echo isset($_GET['searchMascotas']) ? $_GET['searchMascotas'] : ''; ?>">
