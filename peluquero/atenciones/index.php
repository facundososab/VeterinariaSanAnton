...
if (isset($_GET['searchAtenciones']) && !empty($_GET['searchAtenciones'])) {
  ...
  $total_atenciones = $pelu->totalAtencionesXBusqueda($pelu_id, $filtro);
  if ($total_atenciones == 0) {
    $_SESSION['mensaje'] = 'No se encontraron resultados';
    $_SESSION['msg-color'] = 'warning';
  } else {
    $atenciones = $pelu->getAtencionesXBusqueda($pelu_id, $filtro, $empezar_desde, $tamano_paginas);
  }
} else {
  $total_atenciones = $pelu->totalAtenciones($pelu_id);
  $atenciones = $pelu->getAllAtenciones($pelu_id, $empezar_desde, $tamano_paginas);
  if (empty($atenciones)) {
